...
func MachPrefix(machine string) string {
	return string(regex.Find([]byte(machine)))
}

func MachN(machine string) int {
	nStr := strings.TrimLeft(machine, MachPrefix(machine))
...
func ResolveMachines(machines []string, varName string) []string {
	...
	machs := make([]string, len(machines))
	for i, mach := range machines {
		n := MachN(mach)
		machs[i] = machinesDNS[n]
	}
	return machs
}
